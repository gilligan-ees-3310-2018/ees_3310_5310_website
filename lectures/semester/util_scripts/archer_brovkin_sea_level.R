...
make_sea_level_plot_1 <- function() {
  p <- ggplot(sea_level, aes(x = co2,
                             y = sea.level,
                             xmin = co2 - co2.sd,
                             xmax = co2 + co2.sd)) +
...
make_sea_level_plot_2 <- function(annot_size = 10, point_size = 10) {
  p <- ggplot(sea_level_2, aes(x = t,
                               y = sea.level,
                               xmin = t - t.sd,
                               xmax = t + t.sd)) +
    # geom_smooth(method = "lm", fill = NA) +
...
make_sea_level_plot_3 <- function(annot_size = 10, point_size = 10) {
  p <- ggplot(sea_level_2, aes(x = t,
                               y = sea.level,
                               xmin = t - t.sd,
                               xmax = t + t.sd)) +
    ...
    labs(x = expression(paste("Change in global average temperature ", (degree * C))),
         y = "Change in sea level (feet)") +
    annotate("text", x = -4, y = -420, hjust = 0, vjust = 0, size = annot_size,
             label = "atop(plain(\"Last glacial maximum\"), atop(plain(\"20,000 years ago\"), 185 * plain(ppm) ~~ plain(CO)[2] ))", parse = TRUE) +
    annotate("text", x = -0.3, y = -5, hjust = 1, vjust = 1, size = annot_size,
             label = "Today") +
    annotate("text", x = 0.5, y = 0, hjust = 0, vjust = 1, size = annot_size,
             label = "atop(plain(\"Eemian\"),atop(plain(\"120,000 years ago\"), 300 * plain(ppm) ~~ plain(CO)[2] ))", parse = TRUE) +
    annotate("text", x = 2.3, y = 85, hjust = 1, vjust = 0, size = annot_size,
             label = "atop(plain(\"Pliocene\"), atop(plain(\"3 million years ago\"), 400 * plain(ppm) ~~ plain(CO)[2]))", parse = TRUE) +
    annotate("text", x = 4, y = 200, hjust = 0, vjust = 1, size = annot_size,
             label = "atop(plain(\"Eocene\"), atop(plain(\"40 million years ago,\"), 1250 * plain(ppm) ~~ plain(CO)[2] ))", parse = TRUE) +
    theme_bw(base_size = 20)
  p
}
