...
load_kaya <- function(data.dir = get('data.dir', envir = globalenv())) {
  population <- read.csv(file.path(data.dir, 'kaya', 'HistoricalPopulationValues.csv'), skip=12, header=T, stringsAsFactors=F)
  ...
  gdppc <- read.csv(file.path(data.dir, 'kaya', 'historicalRealPerCapitaGDPValues.csv'), header=T, skip=12, stringsAsFactors = F)
  countries <- gdppc[,1:4]
  countries[is.na(countries)] <- ''
  countries <- unlist(lapply(seq_len(nrow(countries)), function(i) str_trim(paste(countries[i,], collapse=''))))
  ...
  energy <- read_excel(file.path(data.dir, 'kaya', 'bp-statistical-review-of-world-energy-2015-workbook.xlsx'),
  ...
  carbon <- read_excel(file.path(data.dir, 'kaya', 'bp-statistical-review-of-world-energy-2015-workbook.xlsx'),
